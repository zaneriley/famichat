...
### User Roles
1. **Neighborhood admin** (instance operator): manages the server; can revoke any device for any user on the instance
2. **Household admin** (parent or guardian): manages family members and their devices; approves pending device additions for non-admin household members
3. **Adult family member**: standard member with full messaging access; self-approving when adding a new device via passkey
4. **Non-admin member** (teen, child): can add devices but requires household-admin approval (passkey path) or must possess an existing trusted device (QR path)

The terms "Admin", "Member", and "Family Admin" used elsewhere in this document map to neighborhood admin, adult member, and household admin respectively. "Household" is the canonical governance term in code; "family" is acceptable in UI copy.
...
Two paths exist for adding a new device to an account. They differ in trust signal and approval requirement.

### Path A — Passkey login

The user authenticates with a passkey (WebAuthn). Approval outcome depends on role:

| User role | Outcome |
|-----------|---------|
| Adult household admin | Immediately approved |
| Adult non-admin member | Immediately approved (self-approving) |
| Non-admin member (teen, child) | Device enters **pending** state (read-only access) until a household admin approves |
| Grandparent / low-tech user | Seamless if passkey synced via iCloud or Google; falls back to magic link + OTP; household admin receives a passive notification |

### Path B — QR / existing device approval

The user scans a QR code generated by an already-trusted device they physically possess. This is the **higher-trust path** because it provides explicit proof of existing device possession.

- Immediately approved for any role; no pending state; no admin review required.
- Covers: switching platforms where a passkey did not transfer, lost-passkey recovery, and any scenario where the user has physical access to a prior trusted device.
- Maps to the existing pairing-token infrastructure (`pair_qr` / `pair_admin_code` token kinds in `Auth.Onboarding`; QR + admin code flows already built).

### Trust hierarchy for device management

```
Neighborhood admin  →  can revoke any device for any user on the instance
Household admin     →  manages family members' devices; approves pending additions
Adult member        →  manages their own devices (self-approving on Path A)
Non-admin member    →  can add via Path A (pending) or Path B (immediate, requires existing device)
```

### Device removal / revocation

- Any neighborhood or household admin can revoke a device.
- Revocation kills the session immediately and triggers MLS group removal from all conversations (implementation in progress).
- UX: a tombstone message appears in each affected conversation, e.g. "Zane removed iPhone 12 from this conversation".
- Past messages retain forward secrecy; the revoked device cannot access future messages.

### Common scenarios

| Scenario | Path | Outcome |
|----------|------|---------|
| New phone, passkey synced (most common) | A | Auto-approved (admin) or self-approved (adult member) |
| iOS → Android, passkey did not transfer | B | Scan QR from old phone; immediate approval |
| Teen's new phone | A | Pending until parent approves |
| Grandparent's new tablet | A | Seamless if synced; OTP fallback; admin notified passively |
| Lost or stolen phone | Admin revokes old device; new device via A or B | Past messages safe; future messages inaccessible from old device |
...
- ✅ **Device-add authorization model**: Two paths established (2026-03-01) — Path A (passkey, role-gated approval) and Path B (QR from existing device, higher-trust, immediately approved). See "Device Authentication Model" section above.

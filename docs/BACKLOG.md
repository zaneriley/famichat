...
- [ ] Add `boundary` hex package for compile-time domain boundary enforcement — guardrails exist on paper but nothing stops code from violating ownership rules → docs/ia-boundary-guardrails.md | P0-dogfood | agent:ia-consensus
- [ ] Delete `FamichatWeb.SchemaMarkup` — contains hardcoded personal data (name, LinkedIn, Tokyo address) in zero-caller production module → .tmp/2026-03-20-boundary-enforcement/round-1/consensus.md | P0-dogfood | agent:boundary-consensus
...
- [ ] Add `use Boundary` annotation to `Famichat.Crypto.MLS` — adapter-only invariant unenforced; any dev can couple MLS to Chat without compiler warning → .tmp/2026-03-20-boundary-enforcement/round-1/consensus.md | P1-confidence | agent:boundary-consensus
- [ ] Add `Famichat.Chat` to `Famichat.Accounts` boundary deps in `accounts.ex` — FamilyContext and HouseholdMembership reference Chat.Family; will fail `mix boundary.check` → .tmp/2026-03-20-boundary-enforcement/round-1/consensus.md | P1-confidence | agent:boundary-consensus
...
- [ ] Add `Famichat.Crypto.MLS` to deps in `Mix.Tasks.Famichat.BackfillSnapshotMacs` — task calls SnapshotMac directly; undeclared dep fails `mix boundary.check` once Crypto.MLS annotated → .tmp/2026-03-20-boundary-enforcement/round-1/consensus.md | P1-confidence | agent:boundary-consensus
...
- [ ] Replace `exports: :all` in `Famichat.Chat` boundary with explicit export list — structural root cause of all web-layer bypass of Chat internals → .tmp/2026-03-20-boundary-enforcement/round-1/consensus.md | P2-debt | agent:boundary-consensus
- [ ] Add `use Boundary` annotation to `Famichat.Accounts.FamilyContext` — only context-level module with documented dep constraints but no machine-enforced annotation → .tmp/2026-03-20-boundary-enforcement/round-1/consensus.md | P2-debt | agent:boundary-consensus
- [ ] Remove phantom dep `Famichat.Auth.Identity` from `Famichat.Auth.Sessions` boundary — no Identity call sites in sessions.ex; phantom dep misleads dep graph readers → .tmp/2026-03-20-boundary-enforcement/round-1/consensus.md | P2-debt | agent:boundary-consensus
- [ ] Remove stale dep `Famichat.Auth.Households` from `Famichat.Auth.Recovery` boundary — Recovery makes no Households calls; direct Repo pattern on HouseholdMembership instead → .tmp/2026-03-20-boundary-enforcement/round-1/consensus.md | P2-debt | agent:boundary-consensus
...
- [ ] Delete `Famichat.Accounts.Errors`, `FamichatWeb.LiveError`, and `Famichat.Communities` — zero-caller dead code; Communities has unusual Chat dep creating latent transitive coupling → .tmp/2026-03-20-boundary-enforcement/round-1/consensus.md | P2-debt | agent:boundary-consensus
- [ ] Fix stale `alias Famichat.Chat.User` in `GroupConversationPrivileges:18` to `Famichat.Accounts.User` — Chat.User does not exist; latent bug → .tmp/2026-03-20-boundary-enforcement/round-1/consensus.md | P2-debt | agent:boundary-consensus
...
- [ ] Decide: move `Famichat.Chat.Family` schema to `Famichat.Accounts`? — resolves 3 boundary violations simultaneously (Accounts missing Chat dep, Onboarding schema dep, FamilyContext dep); if left in Chat, Accounts/Onboarding carry permanent Chat dep → .tmp/2026-03-20-boundary-enforcement/round-1/consensus.md | agent:boundary-consensus
- [ ] Decide: enforce `Famichat.Auth` facade or accept direct sub-module access? — determines whether FamichatWeb deps should list one Auth entry or eight; blocks final `mix boundary.check` config → .tmp/2026-03-20-boundary-enforcement/round-1/consensus.md | agent:boundary-consensus

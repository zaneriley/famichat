defmodule Famichat.Accounts do
  ...
  def refresh_session(device_id, raw_refresh) do
    with :ok <- rate_limit(:refresh_attempt, device_id, 6, 60 * 60),
         {:ok, device} <- fetch_device(device_id),
         :ok <- ensure_not_revoked(device),
         :ok <- ensure_trust_active(device),
         hash <- Token.hash_token(raw_refresh),
         true <-
           secure_compare(device.refresh_token_hash, hash) ||
             {:error, :invalid_refresh} do
      rotate_refresh(device, raw_refresh)
    else
      ...
      {:error, :invalid_refresh} ->
        maybe_revoke_for_theft(device_id)
  ...
  defp rotate_refresh(device, _raw_refresh) do
    Repo.transaction(fn ->
      with {:ok, device} <- touch_device(device),
           {:ok, tokens} <-
             issue_session_tokens(Repo.preload(device, :user).user, device) do
        telemetry(:session, :refresh, %{
  ...
  defp touch_device(device) do
    device
    |> UserDevice.changeset(%{last_active_at: DateTime.utc_now()})
    |> Repo.update()
  end
  ...
  defp ensure_trust_active(device, opts \\ [])
  ...
  defp maybe_revoke_for_theft(device_id) do
    case Repo.transaction(fn ->
           with {:ok, device} <- fetch_device(device_id),
             ...
             {:error, _} -> Repo.rollback(:invalid_refresh)
           end
         end) do
      {:ok, :ok} -> {:error, :revoked}
      {:error, :invalid_refresh} -> {:error, :invalid_refresh}
      {:error, reason} -> {:error, reason}
    end
  end
  ...
  def revoke_device(user_id, device_id) do
    ...
    case Repo.one(query) do
      %UserDevice{} = device ->
        ...
        telemetry(:session, :revoke, %{user_id: user_id, device_id: device_id})
  ...
  def verify_access_token(token) when is_binary(token) do
    with {:ok, payload} <-
         ...
         {:ok, device} <- fetch_device(payload["device_id"]),
  ...
  defp upsert_device(user_id, device_id, user_agent, ip, trust?) do
    ...
    case Repo.get_by(UserDevice, device_id: device_id) do
      nil ->
        attrs = Map.put(attrs, :refresh_token_hash, nil)
        %UserDevice{} |> UserDevice.changeset(attrs) |> Repo.insert()
  ...
  defp trusted_until(true),
    do: DateTime.add(DateTime.utc_now(), @refresh_ttl, :second)

  defp trusted_until(_), do: nil

  defp issue_session_tokens(user, device) do
    now = DateTime.utc_now()
    trusted_until = device.trusted_until

    {:ok, refresh_raw, refresh_hash} = generate_refresh_token()
    ...
    access =
      Phoenix.Token.sign(FamichatWeb.Endpoint, @access_salt, access_payload)
  ...
  defp generate_refresh_token do
    raw = :crypto.strong_rand_bytes(48) |> Base.url_encode64(padding: false)
    {:ok, raw, Token.hash_token(raw)}
  end

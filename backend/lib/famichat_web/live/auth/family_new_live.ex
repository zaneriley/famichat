defmodule FamichatWeb.AuthLive.FamilyNewLive do
  ...
  @impl true
  def handle_event("submit-family-name", %{"family_name" => family_name}, socket) do
    case Onboarding.create_family_self_service(family_name, %{remote_ip: socket.assigns.remote_ip}) do
      {:ok, %{setup_token: raw_token}} ->
        ...
        {:noreply,
         push_navigate(socket, to: locale_path(socket, "/families/start/#{raw_token}"))}
      ...
      {:error, %Ecto.Changeset{} = changeset} ->
        if family_name_taken?(changeset) do
          ...
          Logger.warning("[FamilyNewLive] create_family_self_service changeset error: #{inspect(changeset)}")
          {:noreply, assign(socket, :error, :unexpected)}
        end

      {:error, reason} ->
        Logger.warning("[FamilyNewLive] create_family_self_service error: #{inspect(reason)}")
        {:noreply, assign(socket, :error, :unexpected)}
    end
  end

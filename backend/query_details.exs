...
inv_cols = Famichat.Repo.query!("SELECT column_name FROM information_schema.columns WHERE table_name = 'user_invites'")
IO.inspect(Enum.map(inv_cols.rows, fn [c] -> c end), label: "InviteColumns")

sess_cols = Famichat.Repo.query!("SELECT column_name FROM information_schema.columns WHERE table_name = 'user_sessions'")
IO.inspect(Enum.map(sess_cols.rows, fn [c] -> c end), label: "SessionColumns")

users_cols = Famichat.Repo.query!("SELECT column_name FROM information_schema.columns WHERE table_name = 'users'")

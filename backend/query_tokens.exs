...
dcols =
  Famichat.Repo.query!(
    "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'user_devices' ORDER BY ordinal_position"
  )
...
tokens =
  Famichat.Repo.query!(
    "SELECT context, encode(token, 'hex') as token, sent_to, inserted_at FROM user_tokens WHERE context LIKE '%invite%' ORDER BY inserted_at DESC LIMIT 5"
  )

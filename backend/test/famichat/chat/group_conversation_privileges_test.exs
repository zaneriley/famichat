defmodule Famichat.Chat.GroupConversationPrivilegesTest do
  ...
  describe "schema" do
    ...
    test "validates role is admin or member", %{
      group_conversation: group_conversation,
      admin_user: admin_user
    } do
      # Valid admin role
    ...
    test "enforces conversation and user uniqueness", %{
      ...
      assert %{conversation_id_user_id: ["has already been taken"]} =
               errors_on(changeset)
    end
  end
  ...
  describe "telemetry" do
    setup do
      ...
      handler_name =
        "group-conversation-privileges-test-#{:erlang.unique_integer()}"
    ...
    test "emits telemetry events for future create/update operations", %{
      ref: _ref
    } do

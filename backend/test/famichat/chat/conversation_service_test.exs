defmodule Famichat.Chat.ConversationServiceTest do
  ...
  describe "create_direct_conversation/2 self conversations" do
    test "creates a self conversation if not exists" do
      ...
      {:ok, conversation} = ConversationService.create_direct_conversation(user.id, user.id)
    ...
    test "returns existing self conversation when already created" do
      ...
      {:ok, conv1} = ConversationService.create_direct_conversation(user.id, user.id)
      {:ok, conv2} = ConversationService.create_direct_conversation(user.id, user.id)
  ...
  describe "create_direct_conversation/2 distinct conversations" do
    ...
    test "returns existing direct conversation when already created" do
      ...
      {:ok, conv1} = ConversationService.create_direct_conversation(user1.id, user2.id)
      {:ok, conv2} = ConversationService.create_direct_conversation(user1.id, user2.id)
      assert conv1.id == conv2.id
    end
    ...
    test "fails when users are not in the same family" do
      ...
      {:error, reason} = ConversationService.create_direct_conversation(user1.id, user2.id)
    ...
    test "fails when one user does not exist" do
      ...
      {:error, reason} = ConversationService.create_direct_conversation(user.id, Ecto.UUID.generate())
  ...
  describe "list_user_conversations/1" do
    test "returns direct conversations for a given user" do
      ...
      {:ok, _conv} = ConversationService.create_direct_conversation(user1.id, user2.id)

      # List conversations for user1.
      {:ok, conversations} = ConversationService.list_user_conversations(user1.id)
      assert is_list(conversations)
      assert Enum.any?(conversations, fn c -> c.conversation_type == :direct end)

      # Similarly, list for user2.
      {:ok, conversations2} = ConversationService.list_user_conversations(user2.id)
      assert is_list(conversations2)
    end

    test "returns error when user_id is not a binary" do
      assert {:error, :invalid_user_id} = ConversationService.list_user_conversations(123)
    end
  end
end

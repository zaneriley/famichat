defmodule Famichat.Chat.ConversationVisibilityServiceTest do
  ...
  describe "conversation visibility" do
    ...
    test "hide_conversation/2 hides a conversation for a user", %{
      user1: user1,
      conversation1: conversation
    } do
      assert {:ok, updated_convo} =
    ...
    test "hide_conversation/2 is idempotent", %{
      user1: user1,
      conversation1: conversation
    } do
    ...
    test "unhide_conversation/2 unhides a conversation for a user", %{
      user1: user1,
      conversation1: conversation
    } do
      ...
      # Then unhide it
      assert {:ok, updated_convo} =
               ConversationVisibilityService.unhide_conversation(
                 conversation.id,
                 user1.id
               )
    ...
    test "unhide_conversation/2 returns error for non-existent conversation", %{
      user1: user1
    } do
      assert {:error, :not_found} =
               ConversationVisibilityService.unhide_conversation(
    ...
    test "unhide_conversation/2 is idempotent", %{
      user1: user1,
      conversation1: conversation
    } do
      # First hide the conversation

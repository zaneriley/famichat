defmodule Famichat.Content.Schemas.BaseSchemaTest do
  ...
  describe "base schema" do
    ...
    test "URL uniqueness is enforced" do
      ...
      {:ok, _note} = Famichat.Content.create("note", attrs)
      {:error, changeset} = Famichat.Content.create("note", attrs)
    ...
    test "schema_name is required" do
      assert_raise RuntimeError, ":schema_name option is required", fn ->
        defmodule InvalidSchema do
          use Famichat.Content.Schemas.BaseSchema, translatable_type: "invalid"
        end
      end
    end

defmodule Famichat.Content.MarkdownRendering.RendererTest do
  ...
  setup do
    ...
    :ok = Application.put_env(:famichat, :cache, disabled: false)
    on_exit(fn -> Application.delete_env(:famichat, :cache) end)
    Cache.clear()
    :verify_on_exit!
    :ok
  end
  ...
  describe "render_and_cache/4" do
    ...
    test "caches content after first render" do
      ...
      refute Famichat.Cache.exists?(cache_key)
      {:ok, html1} = Renderer.render_and_cache(markdown, :note, "test_3")
      assert Famichat.Cache.exists?(cache_key)
    ...
    test "force refresh option bypasses cache" do
      ...
      {:ok, true} = Famichat.Cache.put(cache_key, "Old content")

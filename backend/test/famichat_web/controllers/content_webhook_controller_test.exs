defmodule FamichatWeb.ContentWebhookControllerTest do
  ...
  setup do
    Application.put_env(:famichat, :content_repo_url, @content_repo_url)
    :ok
  end
  ...
  describe "handle_webhook/3" do
    test "triggers update for push event with relevant changes", %{conn: conn} do
      ...
      assert {:ok, :updated} =
               FamichatWeb.ContentWebhookController.handle_webhook(
                 conn,
                 payload,
                 []
               )

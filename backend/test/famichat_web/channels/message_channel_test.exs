defmodule FamichatWeb.MessageChannelTest do
  ...
  describe "channel join - legacy format" do
    setup do
      token = Phoenix.Token.sign(@endpoint, @salt, @valid_user_id)
    ...
    test "successfully joins channel with valid token", %{socket: socket} do
      Logger.debug(
        "Attempting to join channel with socket assigns: #{inspect(socket.assigns)}"
      )

      # First assert the join is successful
      assert {:ok, _reply, _socket} =
               subscribe_and_join(socket, MessageChannel, "message:lobby")
  ...
  describe "message broadcasting - legacy format" do
    setup do
      token = Phoenix.Token.sign(@endpoint, @salt, @valid_user_id)
      {:ok, socket} = connect(UserSocket, %{"token" => token})
